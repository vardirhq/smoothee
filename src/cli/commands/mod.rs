...
//!
//! Phase 1 ships `status`. The remaining MVP commands are declared in the CLI
//! surface so `--help` reflects the real product shape, and each reports its
//! roadmap phase rather than pretending to work.
...
pub fn not_yet_implemented(command: &str, phase: &str) -> Result<()> {
    ...
    println!(
        "  {}",
        output::label("Today, `smoothee status` is available.")
    );
    Ok(())
}
